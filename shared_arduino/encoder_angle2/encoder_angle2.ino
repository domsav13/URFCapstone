...
const float HYSTERESIS = 0.5;  // ✅ Dead zone to prevent stuttering
...
void moveMotor(float target) {
    ...
    if (abs(currentAngle - target) > POSITION_TOLERANCE) {
        if (steps > 0) {
            // ✅ Added HYSTERESIS to avoid constant micro-adjustments
            while (abs(readEncoderAngle() - target) > HYSTERESIS) {
                digitalWrite(STEP_PIN, HIGH);
                delayMicroseconds(1000);  // ✅ Increased pulse width for more stable movement
