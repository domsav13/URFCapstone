...
int main(void) {
    ...
    // No handshake/wait for "Ready"—Arduino isn't printing anything now
    ...
    // Open FIFO read-end and a non-blocking write-end dummy
    ...
    while (1) {
        ...
        if (n > 0) {
            cmd[n] = '\0';
            if (cmd[n - 1] == '\n') cmd[n - 1] = '\0';
            // Forward the raw command straight to Arduino
            sendCmd(sd, cmd);
        }
